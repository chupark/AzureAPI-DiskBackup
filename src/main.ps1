...
Import-Module -Name .\lib\StorageOperation.psm1 -Force
...
$StorageTable = StorageTableFromAD -azAdInfo $AzAdInfo `
                                   -storageAccoutResourceGroup $config.storageTable.resourceGroup `
                                   -storageAccoutName $config.storageTable.storageName

# Blob
$StorageBlob = StorageBlobFromAD -azAdInfo $AzAdInfo `
                                 -storageAccoutResourceGroup $config.storageBlob.resourceGroup `
                                 -storageAccoutName $config.storageBlob.storageName
...
$promiseUrl = $snapshotPromise.Headers.'Azure-AsyncOperation'[0]
...
if($snapshotObject.StatusCode -eq 200) {
    ...
    $sasPromiseUrl = $grantSasPromise.Headers.'Azure-AsyncOperation'[0]
    $sasPromise = $null
    do {
        $header = @{
            "Authorization"="bearer " + $azAdInfo.token.access_token
...
if($startBlobCopy.StatusCode -ge 200) {
    ...
    $originBlobCopyId = $startBlobCopy.Headers.'x-ms-copy-id'[0]
    $body | Add-Member -NotePropertyName "PartitionKey" -NotePropertyValue $startBlobCopy.Headers.'x-ms-copy-id'[0]
    $body | Add-Member -NotePropertyName "copyId" -NotePropertyValue $startBlobCopy.Headers.'x-ms-copy-id'[0]
    $body | Add-Member -NotePropertyName "RowKey" -NotePropertyValue $snapshotVhd
    $body | Add-Member -NotePropertyName "xmsCopyStatus" -NotePropertyValue $startBlobCopy.Headers.'x-ms-copy-status'[0]
    $body | Add-Member -NotePropertyName "Timestamp" -NotePropertyValue $startBlobCopy.Headers.Date[0]
    ...
    if($blobCopyStatus.StatusCode -ge 200) {
        ...
        $destBlobCopyId = $blobCopyStatus.Headers.'x-ms-copy-id'[0]

        if($originBlobCopyId -eq $destBlobCopyId) {
            
            do {
                $blobCopyStatus = $StorageBlob.getBlobCopyStatus($blobCopyInfo.value[0].blobContainer, $blobCopyInfo.value[0].vhdName)
                $percent = $blobCopyStatus.Headers.'x-ms-copy-progress'[0].Split("/")
                $output = "{0} | {1} : {2} | {3:P2}" -f (Get-Date), $blobCopyStatus.Headers.'x-ms-copy-status'[0], $blobCopyStatus.Headers.'x-ms-copy-progress'[0], ($percent[0] / $percent[1])
                Write-Output $output
                Start-Sleep -Seconds 5
            } until ($blobCopyStatus.Headers.'x-ms-copy-status'[0] -eq "success")
            $blobCopyStatus.Headers.'x-ms-copy-id'[0]

            [PSCustomObject]$body = [PSCustomObject]::new()
            $body | Add-Member -NotePropertyName "PartitionKey" -NotePropertyValue $blobCopyStatus.Headers.'x-ms-copy-id'[0]
            $body | Add-Member -NotePropertyName "copyId" -NotePropertyValue $blobCopyStatus.Headers.'x-ms-copy-id'[0]
            $body | Add-Member -NotePropertyName "RowKey" -NotePropertyValue $blobCopyInfo.value[0].snapshotName
            $body | Add-Member -NotePropertyName "xmsCopyStatus" -NotePropertyValue $blobCopyStatus.Headers.'x-ms-copy-status'[0]
            $body | Add-Member -NotePropertyName "Timestamp" -NotePropertyValue $blobCopyStatus.Headers.'x-ms-copy-completion-time'[0]
            ...
            $body | Add-Member -NotePropertyName "ContentLength" -NotePropertyValue $blobCopyStatus.Headers.'Content-Length'[0]
...
do {
    ...
    $promise = Invoke-WebRequest -Method Get `
                                 -Headers $header `
                                 -Uri $requestRevokeAccess.Headers.'Azure-AsyncOperation'[0] `
                                 -UseBasicParsing
