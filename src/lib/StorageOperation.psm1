Class StorageBlob {
    ...
    [PSCustomObject]startStoragePageBlobCopyFromURL([String]$url) {
        $copyDate = [System.DateTime]::UtcNow.ToString("R")
        $canonicalizedHeaders = "x-ms-copy-source:" + $url       + "`n" + 
                                "x-ms-date:" + $copyDate                    + "`n" +
                                "x-ms-version:2018-11-09"               + "`n"
                                 
        $StringToSign = "PUT`n`n`n`n" + $CanonicalizedHeaders +"/pcwstoragetable/blob/test2.vhd"
        ...
        $invokeResult = Invoke-WebRequest -Method Put -Uri "https://pcwstoragetable.blob.core.windows.net/blob/test2.vhd" -Headers $copyHeader
